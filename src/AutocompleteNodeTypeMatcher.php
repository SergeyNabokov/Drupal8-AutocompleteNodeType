...
/**
 * @file
 * Contains \Drupal\autocomplete_node_type\EntityAutocompleteMatcher.
 */

namespace Drupal\autocomplete_node_type;
 ...
 */
class AutocompleteNodeTypeMatcher {
  ...
  public function getMatches($target_type, $selection_handler, $selection_settings, $string = '') {
    ...
    if (isset($string)) {
      ...
      foreach ($entity_labels as $node_type_name => $values) {
        foreach ($values as $entity_id => $label) {
          ...
          $key = Tags::encode($key);
          $matches[] = array('value' => $key, 'label' => $this->getLabel($target_type, $node_type_name, $label)); 
        }
      }
    }
  ...
  /**
   * Gets matched label based on a given target type.
   * 
   * @param string $target_type
   *   The ID of the target entity type.
   * 
   * @param string $node_type_name
   *   The node type of the entity.
   * 
   * @param string $label
   ...
   *   An entity label
   */
  public function getLabel($target_type, $node_type_name, $label) {
    if ($target_type === 'node') {
      $node_type = node_type_load($node_type_name);
